...
- **Arquitectura**: cuatro archivos. `main.py` (app FastAPI: endpoints en español — `/analizar/`, `/agregar_trabajo/`, `/clientes/`, `/analisis/` —, extracción de texto, match semántico por fragmentos con `paraphrase-multilingual-MiniLM-L12-v2`, puntaje híbrido 0.4·semántico + 0.6·LLM), `prompts.py` (la "guía de trabajo" del LLM: plantillas completas por idioma es/en — el feedback sale en el idioma del CV), `database.py` (modelos SQLAlchemy en español: `Cliente` → `Trabajo` → `Habilidad`/`Funcion`/`Perfil`/`Analisis`; la tabla `analisis` guarda el historial con `raw_score` y `puntaje_llm` para recalibrar), e `interfaz.py` (Streamlit, consume la API por HTTP).
- **Comandos**: `uvicorn main:app --reload` (API), `streamlit run interfaz.py` (interfaz, puerto 8501), `alembic upgrade head` (migraciones). No hay tests ni linter.
